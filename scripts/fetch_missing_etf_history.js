...
// Função utilitária para buscar todos os registros paginando
...
async function fetchAllSymbols() {
  const rows = await fetchAllRows('etf_list', 'symbol');
  return rows.map(row => row.symbol);
}

async function fetchSymbolsWithPrices() {
  const rows = await fetchAllRows('etf_prices', 'symbol');
  // Pode haver duplicatas, então usar Set
  return Array.from(new Set(rows.map(row => row.symbol)));
}
...
async function main() {
  ...
  const symbolsWithPrices = await fetchSymbolsWithPrices();
  const missingSymbols = allSymbols.filter(s => !symbolsWithPrices.includes(s));
  console.log(`Total de ETFs na etf_list: ${allSymbols.length}`);
  console.log(`ETFs já com histórico em etf_prices: ${symbolsWithPrices.length}`);
  console.log(`ETFs faltantes para buscar: ${missingSymbols.length}`);

  let totalPrices = 0, totalDivs = 0, totalErrors = 0;

  for (let idx = 0; idx < missingSymbols.length; idx++) {
    const symbol = missingSymbols[idx];
    try {
      console.log(`[${idx + 1}/${missingSymbols.length}] Buscando histórico para ${symbol}...`);
      // Preços históricos
